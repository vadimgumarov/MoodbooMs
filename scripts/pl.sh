...
pl() {
    ...
    if [ ! -z "$last_epoch" ]; then
        ...
        if [ ! -z "$last_commit" ]; then
            ...
            modified_files=$(git diff --name-only HEAD~5 HEAD 2>/dev/null | grep -E "\.(js|jsx|ts|tsx|json|sh)$" | head -5 | sed 's/^/  - /')
        fi
    else
        modified_files=$(git diff --name-only HEAD~5 HEAD 2>/dev/null | grep -E "\.(js|jsx|ts|tsx|json|sh)$" | head -5 | sed 's/^/  - /')
    fi
...
if [ "$modified_count" -gt 0 ]; then
    ...
    git status --porcelain | grep "^ M" | awk '{print $2}' | while read file; do
        ...
        elif echo "$file" | grep -q "\.jsx$"; then
            echo "  - Updated React component: $(basename $file)"
        elif echo "$file" | grep -q "electron/"; then
            echo "  - Modified Electron main process: $(basename $file)"
        else
            echo "  - Modified: $file"
        fi
    done
...
if echo "$new_commits" | grep -q "test"; then
    echo "  - Add E2E tests for critical user flows"
fi
if echo "$modified_files" | grep -q "Calendar\|calendar"; then
    echo "  - Implement calendar month navigation and date selection"
fi
if echo "$modified_files" | grep -q "electron\|main.js"; then
    echo "  - Add secure IPC communication layer"
fi
# Default suggestions if no specific patterns
if [ $(echo "$new_commits" | wc -l) -lt 3 ]; then
    echo "  - Implement data persistence with electron-store"
    echo "  - Create dynamic tray icons for different cycle phases"
    echo "  - Add user onboarding flow for first-time users"
fi
