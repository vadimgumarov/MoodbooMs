...
pytest tests/ --cov=modules --cov=strategies --cov=utils --cov=cli \
    --cov-report=term-missing:skip-covered \
    --tb=short \
    -v

# Check if coverage meets threshold
COVERAGE=$(pytest tests/ --cov=modules --cov=strategies --cov=utils --cov=cli --cov-report=term | grep TOTAL | awk '{print $4}' | sed 's/%//')
if [ -n "$COVERAGE" ]; then
    if (( $(echo "$COVERAGE >= 85" | bc -l) )); then
        echo ""
        echo -e "${GREEN}✅ Coverage: ${COVERAGE}% (meets 85% requirement)${NC}"
    else
        echo ""
        echo -e "${YELLOW}⚠️  Coverage: ${COVERAGE}% (below 85% requirement)${NC}"
    fi
fi
