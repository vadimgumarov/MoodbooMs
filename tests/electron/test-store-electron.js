// Test store functionality in Electron environment
const { app } = require('electron');
const { storeOperations } = require('../../electron/store');

app.whenReady().then(() => {
  console.log('Testing electron-store functionality in Electron environment...\n');
  ...
  console.log('Test 5: Data persistence (checking if data exists from previous runs)');
  try {
    const allData = storeOperations.getAll();
    console.log('✓ Current store contents:', JSON.stringify(allData, null, 2));
    console.log('✓ Test passed!\n');
  } catch (error) {
    console.error('✗ Test failed:', error.message);
  }
